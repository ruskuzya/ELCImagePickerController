...
- (void)preparePhotos
{
    @autoreleasepool {
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            ...
            [self.navigationItem setTitle:self.singleSelection ? NSLocalizedString(@"Выберите фото", nil) : NSLocalizedString(@"Выберите фото", nil)];
        });
    }
}
...
{
    return (self.view.bounds.size.width - 20.0) / 4 + 4.0;
}
